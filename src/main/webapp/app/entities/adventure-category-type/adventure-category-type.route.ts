...
export class AdventureCategoryTypeResolve implements Resolve<IAdventureCategoryType> {
  constructor(private service: AdventureCategoryTypeService, private router: Router) {}

  resolve(route: ActivatedRouteSnapshot): Observable<IAdventureCategoryType> | Observable<never> {
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(
        flatMap((adventureCategoryType: HttpResponse<AdventureCategoryType>) => {
          if (adventureCategoryType.body) {
            return of(adventureCategoryType.body);
          } else {
