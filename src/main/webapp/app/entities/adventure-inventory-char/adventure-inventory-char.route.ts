...
export class AdventureInventoryCharResolve implements Resolve<IAdventureInventoryChar> {
  constructor(private service: AdventureInventoryCharService, private router: Router) {}

  resolve(route: ActivatedRouteSnapshot): Observable<IAdventureInventoryChar> | Observable<never> {
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(
        flatMap((adventureInventoryChar: HttpResponse<AdventureInventoryChar>) => {
          if (adventureInventoryChar.body) {
            return of(adventureInventoryChar.body);
          } else {
