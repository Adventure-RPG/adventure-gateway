...
export class AdventureModelResolve implements Resolve<IAdventureModel> {
  constructor(private service: AdventureModelService, private router: Router) {}

  resolve(route: ActivatedRouteSnapshot): Observable<IAdventureModel> | Observable<never> {
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(
        flatMap((adventureModel: HttpResponse<AdventureModel>) => {
          if (adventureModel.body) {
            return of(adventureModel.body);
          } else {
