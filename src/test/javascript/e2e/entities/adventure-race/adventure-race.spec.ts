...
describe('AdventureRace e2e test', () => {
  ...
  it('should create and save AdventureRaces', async () => {
    ...
    await promise.all([
      ...
      adventureRaceUpdatePage.adventureAccountCharacterSelectLastOption()
    ]);
    expect(await adventureRaceUpdatePage.getNameInput()).to.eq('name', 'Expected Name value to be equals to name');
    expect(await adventureRaceUpdatePage.getDescInput()).to.eq('desc', 'Expected Desc value to be equals to desc');
    expect(await adventureRaceUpdatePage.getImageInput()).to.endsWith(
      fileNameToUpload,
      'Expected Image value to be end with ' + fileNameToUpload
    );
    await adventureRaceUpdatePage.save();
    expect(await adventureRaceUpdatePage.getSaveButton().isPresent(), 'Expected save button disappear').to.be.false;

    expect(await adventureRaceComponentsPage.countDeleteButtons()).to.eq(nbButtonsBeforeCreate + 1, 'Expected one more entry in the table');
  });
