...
describe('AdventureAttributes e2e test', () => {
  ...
  it('should create and save AdventureAttributes', async () => {
    ...
    expect(await adventureAttributesUpdatePage.getDefenceInput()).to.eq('5', 'Expected defence value to be equals to 5');
    expect(await adventureAttributesUpdatePage.getFireResistanceInput()).to.eq('5', 'Expected fireResistance value to be equals to 5');
    expect(await adventureAttributesUpdatePage.getEarthResistanceInput()).to.eq('5', 'Expected earthResistance value to be equals to 5');
    expect(await adventureAttributesUpdatePage.getWaterResistanceInput()).to.eq('5', 'Expected waterResistance value to be equals to 5');
    expect(await adventureAttributesUpdatePage.getWindResistanceInput()).to.eq('5', 'Expected windResistance value to be equals to 5');
    expect(await adventureAttributesUpdatePage.getActiveWeaponAttackHitInput()).to.eq(
      '5',
      'Expected activeWeaponAttackHit value to be equals to 5'
    );
    expect(await adventureAttributesUpdatePage.getActiveWeaponAttackTypeInput()).to.eq(
      '5',
      'Expected activeWeaponAttackType value to be equals to 5'
    );
    expect(await adventureAttributesUpdatePage.getSizeInput()).to.eq('5', 'Expected size value to be equals to 5');
    await adventureAttributesUpdatePage.save();
    expect(await adventureAttributesUpdatePage.getSaveButton().isPresent(), 'Expected save button disappear').to.be.false;

    expect(await adventureAttributesComponentsPage.countDeleteButtons()).to.eq(
