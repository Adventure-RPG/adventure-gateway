...
describe('AdventureRaceOptions e2e test', () => {
  ...
  it('should load AdventureRaceOptions', async () => {
    ...
    await browser.wait(ec.visibilityOf(adventureRaceOptionsComponentsPage.title), 5000);
  ...
  it('should load create AdventureRaceOptions page', async () => {
    ...
    expect(await adventureRaceOptionsUpdatePage.getPageTitle()).to.eq('adventureGatewayApp.adventureRaceOptions.home.createOrEditLabel');
  ...
  it('should create and save AdventureRaceOptions', async () => {
    ...
    await promise.all([
      ...
      adventureRaceOptionsUpdatePage.adventureRaceSelectLastOption()
    ]);
    expect(await adventureRaceOptionsUpdatePage.getHeightInput()).to.eq('5', 'Expected height value to be equals to 5');
    expect(await adventureRaceOptionsUpdatePage.getWeightInput()).to.eq('5', 'Expected weight value to be equals to 5');
    await adventureRaceOptionsUpdatePage.save();
    expect(await adventureRaceOptionsUpdatePage.getSaveButton().isPresent(), 'Expected save button disappear').to.be.false;

    expect(await adventureRaceOptionsComponentsPage.countDeleteButtons()).to.eq(
